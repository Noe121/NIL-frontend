...
const MultiStepRegister_WithCompliance = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    // Step 2.5: User Type Selection (for athletes)
    userType: '',
    
    // Step 3: State Selection (for athletes)
    state: '',
    
    // Step 4: Compliance Info (for athletes)
    age: '',
    school: '',
    sport: '',
    
    // Step 5: Role-specific Info
    // Athlete
    team: '',
    position: '',
    year: '',
  ...
  // Step flow depends on role
  const getMaxSteps = () => {
    if (formData.role === 'athlete') {
      return 6; // Basic Info -> Role -> User Type -> State -> Compliance Info -> Role-specific
    }
    return 3; // Basic Info -> Role -> Role-specific
  };
  ...
  const getStepLabel = () => {
    ...
    if (currentStep === 2) return 'Choose Your Role';
    if (currentStep === 3 && formData.role === 'athlete') return 'Select Athlete Category';
    if (currentStep === 4 && formData.role === 'athlete') return 'Select Your State';
    if (currentStep === 5 && formData.role === 'athlete') return 'Compliance Information';
    if (currentStep === 6 && formData.role === 'athlete') return 'Complete Your Profile';
    if (currentStep === 3 && formData.role !== 'athlete') return 'Tell Us More';
    return 'Register';
  };
  ...
  const validationRules = {
    ...
    3: formData.role === 'athlete' ? {
      userType: [(value) => value ? null : 'Please select your athlete category']
    } : getStep3ValidationRules(),
    4: formData.role === 'athlete' ? {
      state: [(value) => value ? null : 'Please select a state']
    } : {},
    5: formData.role === 'athlete' ? {
      age: [validators.age],
      school: [validators.required],
      sport: [validators.sport]
    } : {},
    6: formData.role === 'athlete' ? getStep3ValidationRules() : {}
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // First validate compliance for athletes
      if (formData.role === 'athlete' && formData.state && formData.age && formData.userType) {
  ...
  const getProfileData = () => {
    ...
    return { ...baseProfile, ...roleSpecific };
  };
      ...
      <div className="max-w-md w-full bg-white rounded-lg shadow-lg p-8">
        ...
        <form onSubmit={handleSubmit}>
          <AnimatePresence mode="wait">
            ...
            {/* Step 3: User Type Selection (Athletes Only) */}
            {currentStep === 3 && formData.role === 'athlete' && (
              <motion.div
                key="step3-user-type"
                variants={stepVariants}
                initial="hidden"
            ...
            {/* Step 4: State Selection (Athletes Only) */}
            {currentStep === 4 && formData.role === 'athlete' && (
              <motion.div
                key="step4-state"
                variants={stepVariants}
                initial="hidden"
            ...
            {/* Step 5: Compliance Information (Athletes Only) */}
            {currentStep === 5 && formData.role === 'athlete' && (
              <motion.div
                key="step5-compliance"
                variants={stepVariants}
                initial="hidden"
                animate="visible"
                exit="exit"
            ...
            {/* Step 3/6: Role-specific Information */}
            {((currentStep === 3 && formData.role !== 'athlete') || (currentStep === 6 && formData.role === 'athlete')) && (
              <motion.div
                key="step-role-specific"
                variants={stepVariants}
                initial="hidden"
                animate="visible"
                exit="exit"
...
export default MultiStepRegister_WithCompliance;
